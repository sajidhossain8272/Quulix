  ...
  return (
    <div className="space-y-6">
      ...
      <label className="block space-y-2">
        <span className="text-sm font-medium text-stone-800">Search within this page</span>
...
export function CategoryPageClient({
  ...
  const [search, setSearch] = useState(readValue(initialSearchParams.search) ?? "");
  ...
  const [minPrice, setMinPrice] = useState(readValue(initialSearchParams.minPrice) ?? "");
  const [maxPrice, setMaxPrice] = useState(readValue(initialSearchParams.maxPrice) ?? "");
  const [minRating, setMinRating] = useState(readValue(initialSearchParams.minRating) ?? "");
  ...
  useEffect(() => {
    ...
    startTransition(() => {
      router.replace(`/category/${slug}${queryString ? `?${queryString}` : ""}`, {
        scroll: false,
      });
    });
  }, [
  ...
  const category = categoriesQuery.data?.data.find((item) => item.slug === slug);
  const title = slug === "all" ? "All Categories" : category?.name ?? "Catalog";
  const description =
    slug === "all"
      ? "Browse the full Auralux Market catalog with mobile-first filters and dense product discovery."
      : category?.description ?? "Refined products curated for premium everyday routines.";
        ...
        <section className="overflow-hidden rounded-[32px] border border-stone-200 bg-white/85 p-6 shadow-[0_22px_70px_rgba(15,23,42,0.06)] sm:p-8 lg:p-10">
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-stone-500">
              ...
              <span className="rounded-full bg-stone-100 px-4 py-2">
                {productsQuery.isFetching ? "Refreshing catalog" : "Cached via React Query"}
              </span>
            </div>
          </div>
        </section>
