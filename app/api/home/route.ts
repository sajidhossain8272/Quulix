...
export async function GET() {
  ...
  const heroSlides = categories.slice(0, 3).map((cat, index) => ({
    ...
    description: cat.description || `Discover the best in ${cat.name}.`,
    ctaLabel: `Shop ${cat.name}`,
    ctaHref: `/category/${cat.slug}`,
    image: cat.image || "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=1200&q=80",
    alt: cat.name,
  }));
